...
class GetGamesForYouUseCase(IGameForYou):
    ...
    async def get_games(self,session,count_dict,appid_list,page:int=1,limit:int=10):
        ...
        data = await self.analitic_repository.games_for_you(session=session, ganres_data=count_dict.get("ganres_dict"),category_data=count_dict.get("categories_dict"),
                                                   steam_appids=appid_list,page=page,limit=limit)
        return data
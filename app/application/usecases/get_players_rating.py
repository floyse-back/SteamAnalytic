...
class GetUserRatingUseCase:
    async def execute(self,data:SteamUser) -> int:
        year_create_user_score = await self.__check_user_years(data["user_data"]["player"]["timecreated"]) * 10
        ...
        count_game_time = await self.__game_check_result(games=data["user_games"]["games"]) if data["user_games"].get("games") else 0
    ...
    @staticmethod
    async def __check_user_years(seconds):
    ...
    @staticmethod
    async def __game_check_result(games:list)->int:
        result = 0
        game_time_forever=0
        game_time_2weeks = 0
        for game in games:
